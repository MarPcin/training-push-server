...
function sendNtfy(topic, name, customTitle, customMessage) {
  const payload = JSON.stringify({
    topic,
    title: customTitle || '\u23f1 Rest done!',
    message: customMessage || ('Time to get back to ' + name + ' \u2014 next set!'),
    priority: 4,
    tags: ['muscle'],
    icon: 'https://marpcin.github.io/Training-App/apple-touch-icon-180.png'
  });
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && parts[0] === 'schedule-ntfy') {
    ...
    req.on('end', () => {
      try {
        const { delayMs, topic, name, title, message } = JSON.parse(body);
        ...
        ntfyTimers[topic] = setTimeout(() => {
          ...
          sendNtfy(topic, name || 'exercise', title, message);
        }, delay);
